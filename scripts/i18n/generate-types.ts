...
async function generateTypesScript() {
  try {
    ...
    const outputDir = path.resolve(
      __dirname,
      '../../src/shared/config/i18n/generated',
    )
    ...
    if (languages.length === 0) {
      console.error('No languages found in public/locales')
      process.exit(1)
    }
    ...
    for (const file of jsonFiles) {
      ...
      const interfaceCode = generateInterface(data, interfaceName)
      namespaceInterfaces.push(interfaceCode.trim())

      namespacePaths[namespace] = generateKeyPaths(data)
    }

    const resourcesInterface = `interface Resources {\n${Object.keys(resources)
      .map(
        (ns) =>
          `  ${ns}: ${ns.charAt(0).toUpperCase()}${ns.slice(1)}Resources`,
      )
      .join('\n')}\n}`
...
export type KeysForNamespace<NS extends keyof Resources> = ${Object.keys(namespacePaths)
      .map(
        (ns) =>
          `  NS extends '${ns}' ? ${ns.charAt(0).toUpperCase()}${ns.slice(1)}Keys :`,
      )
      .join('\n')}
  never
...
`
    ...
    console.log(
      `Namespaces: ${Object.keys(resources).join(', ')}`,
    )
  } catch (error) {
